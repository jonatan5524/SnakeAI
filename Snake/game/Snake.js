...
class Snake {
    constructor(size, brain) {
        ...
        this.head = new SnakeCell(INIT_X / BLOCK_SIZE, INIT_Y / BLOCK_SIZE, BLOCK_SIZE, this.color);
        this.body = [];
    ...
    appleDagree(apple) {
        if (this.decision == UP || this.decision == DOWN) {
            ...
            return map(Math.atan((this.head.col - apple.col) /
        ...
        return map(Math.atan(Math.abs(this.head.row - apple.row) /
        ...
        switch (this.direction) {
            case UP:
                return (((this.head.row - 1 < 0) ||
                    (this.body.find((cell) =>
                        cell.col == this.head.col &&
                        cell.row == this.head.row - 1) != undefined)) ? 1 : 0);

            case RIGHT:
                return (((this.head.col + 1 >= (canvas_width / BLOCK_SIZE)) ||
                    (this.body.find((cell) =>
                        cell.col == this.head.col + 1 &&
                        cell.row == this.head.row) != undefined)) ? 1 : 0);
    ...
    appleDistance(apple) {
        return this.pitagoras_third_vertex(this.head.col - apple.col, this.head.row - apple.row);
    }

    appleScore(oldCol, oldRow, apple) {
        if (this.appleDistance(apple) <
    ...
    appleOnBody(apple) {
        return (this.body.find((cell) => cell.col == apple.col &&
            cell.row == apple.row) != undefined);
    }

    checkEnd() {
        return (this.head.col * this.head.size <= -1 * this.head.size ||
